...
let repository_main_branch = "master"; // TODO: rename to main when the public repository exists
  ...
  if !ensure_open(workspace_folder) {
    ...
    Repository::clone(repository_url, workspace_folder).expect("failed to clone repository: {}");
    println!(" Done!");
  }
...
fn pull_latest(workspace_folder: &str, remote_branch: &String) -> () {
  ...
  origin.fetch(&[remote_branch], None, None).expect("Fetch origin failed");
  ...
  if analysis.0.is_up_to_date() {
      ...
      let mut reference = repo.find_reference(&refname).expect("Could not find reference branch");
      reference.set_target(fetch_commit.id(), "Fast-Forward").expect("Could not set target");
      repo.set_head(&refname).expect("Could not set head");
      repo.checkout_head(Some(git2::build::CheckoutBuilder::default().force())).expect("Could not checkout head");
